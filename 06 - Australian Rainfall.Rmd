...
The machine doesn't know what the regions are so it just gives each region a number. But we can examine the groupings to see that its picked out areas like Victoria and Tasmania south of the Great Divide, South Australia and Victoria north of the Great Divide, the farming regions in N.S.W., the Outback and so on.
...
region_dashboard <- function(regionID, start_period, end_period, minor_period, rain_limit = NULL) {
    ...
    trend_plot <-
        ...
        labs(
            x = "Year",
            y = "Median rainfall for last 12 months (mm)"
        ) +
        theme_bw()
...
Getting to this stage has taken a lot of work. Much poking around, coding, stats, plotting and rearranging to get a clear signal out of those half a billion rows of data spread over 17,000 files.

But after trying a lot of things I think I've found a way to show whats going on thats easy to understand.

Lets see it for region 2.

```{r, fig.asp = 1}
region_dashboard(2, baseline_start, baseline_end, 12, rain_limit = c(100, 800))
```

We can see from the map on this dashboard that this analysis is for 101 weather stations in Southern Western Australia. 

In the bottom right hand is the rainfall for each station over time. Each point represents the total rainfall for the prior 12 months. Each station has its own color which helps give a sense of how all the points are connected. Next I've plotted the median rainfall in black. The median rainfall shows what the "typical" rainfall is for all those stations.

Don't let all the red dots above the median line fool you. Their are an equal number of points below the line as above it but they overlap so much they are hard to see.
...
The blue line is the trend over time. Its pretty damn flat so for this region the answer seems to be no. Its not getting drier.

## Headin' down South!

Lets see the same dashboard for region 3. My home region. Southern Victoria and Tasmania.

```{r, fig.asp = 1}
region_dashboard(3, baseline_start, baseline_end, 12, rain_limit = c(400, 1100))
```

Well here we get quite a different story. You can clearly see the [Millennium drought](https://en.wikipedia.org/wiki/Drought_in_Australia#2000s_or_'Millennium'_drought_in_south-eastern_Australia). Its recovered a bit but in general it looks like we just don't as much rain as we used to. The trend line shows that the typical rainfall has dropped from 800mm a year to around 680mm a year.
...
This region seems to cover a lot of the areas currently in drought in New South Wales.. Because each point is the prior years data and my data only goes up to 2017 its about 2 years behind but it might be showing the start of it.

In general this area has a lower rainfall than Southern Victorian region which is probably why the algorithm grouped them together.

Like the Southern region it shows a downward trend.
...
In general we got more rain in South East Queensland. But again the trend is downward from 800mm to 710mm. It looks like we don't get quite the same peaks anymore.
...
This area convering South Australia and Western Victoria is considerably drier than Eastern Australia but they do grow lots of wheat out there. 

Its doesn't really seem to have a distinct trend. Note that the trend graph is for a smaller range than for the Eastern states so the small trend looks bigger in comparison.

# Conclusion

And I'll leave it there. If you look at the remaining regions they repeat the pattern of Western Australia and South Australia of not really having a trend.

But it seems that its not so much the case that Australia is getting drier, but rather the Eastern states are getting drier.
