...
m_pca <- function(data) {
    ...
    x <- data %>% dplyr::select_if(is.numeric)
    princomp(x)
}

#' Screeplot for Principal Components
#'
#' @param pca             pca analysis
...
#'
m_screeplot <- function(pca, screetype = "pev") {

    pcobj <- pca
    d <- pcobj$sdev ^ 2
    yvar <- switch(screetype,
             pev = d / sum(d),
             cev = cumsum(d) / sum(d))

    yvar.lab <- switch(screetype,
...
#'
m_biplot <- function(m, choices = 1:2, scale = 1, pc.biplot = TRUE,
