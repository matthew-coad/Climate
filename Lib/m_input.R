...
m_divide <- function(data, response = NULL) {
    ...
    if (!is.null(response)) {
        x <- dplyr::select_(data_df, lazyeval::f_interp(~-uq(response)))
        y <- dplyr::select_(data_df, response)
    } 
    else {
        x <- data_df
        y <- NULL
...
m_test_index <- function(m) {
    m_default(m,
        test_index = function(input_data, response = NULL) {
            ...
            y <- dplyr::select_(input_data, response) %>% .[[1]]
            r <- which(is.na(y))
            if (is_empty(r)) {
                return (NULL)
            }
...
m_stratified_split <- function(m, p = 0.8) {
    ...
    m_set(
        ...
        data_index = function(input_data, response, seed, split_p, test_index = NULL) {
            ...
            set.seed(seed)
            y <- input_data %>% dplyr::select_(response)
